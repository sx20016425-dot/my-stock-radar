...
@st.cache_data(ttl=300) # 每 5 分鐘快取一次，避免頻繁請求美股導致變慢
...
def run_trading_engine(symbol, q):
    ...
    while retry_count < 3:
        try:
            q.put({"type": "log", "content": f"正在嘗試連線... (第 {retry_count+1} 次)"})
            ...
            q.put({"type": "log", "content": f"✅ 監控通道已成功開啟"})
            while True: 
                time.sleep(10) # 保持執行緒存活
                if not stock.is_connected(): # 簡易斷線檢查
                    raise Exception("Connection lost")
        except Exception as e:
            retry_count += 1
            q.put({"type": "log", "content": f"⚠️ 連線失敗: {str(e)}，5秒後重試..."})
            time.sleep(5)
...
with idx_c1: st.metric("台股加權 (大盤)", f"{st.session_state.index_data['price']}", f"{st.session_state.index_data['percent']}%")
with idx_c2: st.metric("TSM (台積ADR)", f"{us_indices['TSM']['price']:.2f}", f"{us_indices['TSM']['percent']:.2f}%")
with idx_c3: st.metric("NVDA (輝達)", f"{us_indices['NVDA']['price']:.2f}", f"{us_indices['NVDA']['percent']:.2f}%")
with idx_c4: st.metric("費城半導體", f"{us_indices['^SOX']['price']:.1f}", f"{us_indices['^SOX']['percent']:.2f}%")
...
if start_btn:
    ...
    while True:
        ...
        if test_mode:
            ...
            st.session_state.msg_queue.put({"type": "stock_trade", "price": p, "size": random.randint(1, 20), "side": random.choice(["buy", "sell"])})
        ...
        while not st.session_state.msg_queue.empty():
            ...
            if item["type"] == "log": st.session_state.logs.insert(0, item["content"])
                ...
                if p_str not in st.session_state.price_stats: st.session_state.price_stats[p_str] = {'買入': 0, '賣出': 0}
        ...
        if st.session_state.price_stats:
            ...
            power_ui.metric("多空力道", f"{int(t_buy)} / {int(t_sell)}", f"{int(t_buy-t_sell)} 淨量")
